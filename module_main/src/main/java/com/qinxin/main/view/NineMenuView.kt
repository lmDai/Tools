...
import com.qinxin.main.adapter.NineViewPagerAdapter
import kotlinx.android.synthetic.main.nine_memu_main.view.*
import kotlin.math.ceil

class NineMenuView(var content: Context, fragment: FragmentActivity) : FrameLayout(content) {
    ...
    private val nineViewPagerAdapter: NineViewPagerAdapter by lazy {
        NineViewPagerAdapter(
            fragment,
            menuList,
            pageSize
        )
    ...
    fun setData(data: List<MenuBean>) {
        ...
        if (points.size != totalPage) {
            ...
            for (current in 1..totalPage) {
                ...
                if (current == 1) {
                    imageView.setBackgroundResource(com.qinxin.common.R.drawable.selected_indicator)
                } else {
                    imageView.setBackgroundResource(com.qinxin.common.R.drawable.normal_indicator)
                }
    ...
    private fun selectedPoints(position: Int) {
        for (index in 1..points.size) {
            if (index == position) {
                points[index - 1].setBackgroundResource(com.qinxin.common.R.drawable.selected_indicator)
            } else {
                points[index - 1].setBackgroundResource(com.qinxin.common.R.drawable.normal_indicator)
            }
        }
    }
}